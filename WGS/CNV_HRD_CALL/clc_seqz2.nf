...
process bam_index {
    ...
    output:
        tuple val(sample_id), path("${normal_bam}"), path("${normal_bam}.bai"), emit: normal_bam_bai
        tuple val(sample_id), path("${tumor_bam}") , path("${tumor_bam}.bai"), emit: tumor_bam_bai
...
process SEQZ_BINNING {
    ...
    output:
        tuple val(sample_id), path("${sample_id}_bin50.seqz.gz"), emit: smallbinfile
...
//
//    output:
//        tuple val(sample_id), path("${sample_id}_bin50.seqz"), emit: ready_to_use_smallbinfile
//
//    script:
//    """
//    bgzip -d ${smallbinfile} -o ${sample_id}_bin50.seqz
//    """
//}
...
process R_SEQUENZA { 
    ...
    //conda "bioconda::r-sequenza=3.0.0=r41h3342da4_4"
...
workflow sequenza {
   ...
   main:
       bam_index(csv_ch)
       bam2seqz(bam_index.out.normal_bam_bai,bam_index.out.tumor_bam_bai,ref_fasta_gz_ch,gc_wig_file_ch)
