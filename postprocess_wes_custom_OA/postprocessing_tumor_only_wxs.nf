...
process bam_qc {
    ...
    if [[ "${sample_name}" =~ [T] && "${library_type}" == "wes" ]]; then
        cov_value=100
...
process index_filtered_bams {
    ...
    input:
        tuple val(sample_ID), path(filtered_tumorbam)

    output:
        tuple val(sample_ID), path("${filtered_tumorbam}"), path("${filtered_tumorbam}.bai"), emit: idx_filtered_bam
...
process msisensor_pro {
    ...
    conda "bioconda::msisensor-pro=1.3.0"

    publishDir "${params.outdir}/${params.library_type}_${sample_ID}/msisensor_pro", mode: "copy"

    input:
        tuple val(sample_ID), path(idx_filtered_tumorbam), path(idx_filtered_tumorbai) 
    ...
    output:
        tuple val(sample_ID), path("${idx_filtered_tumorbam.getSimpleName()}_msi.csv")
    ...
    msisensor-pro pro \\
        ...
        -o ${idx_filtered_tumorbam.getSimpleName()}_msi.csv
    """
}
...
workflow postprocessing_wes_custom_oa {
    ...
    main:
       ...
       // Index bam
       idx_filtered_bam = index_filtered_bams(bam_qc.out.filtered_bam)
       //idx_filtered_bam.view()

       // Msisensor pro
       msisensor_pro(idx_filtered_bam,baseline_ch)
...
workflow {
    ...
    baseline_ch = Channel.value(params.msi_ref_baseline)
