...
process process_bamfile {
    ...
    echo "min_cov,max_cov,mean_cov,above_mincov" > header.csv
    ...
    echo \$full_info | jq -Rsn '
                         {"bam_qc":
                           [inputs
                            | . / "\n"
                            | (.[] | select(length > 0) | . / ",") as \$input
                            | {"file": \$input[0], "min_cov": \$input[5], "max_cov": \$input[6], "mean_cov": \$input[7], "above_mincov": \$input[8]}]}
                              ' > ${bam.getSimpleName()}_bam.json

    samtools depth -b ${grz_bed} ${bam} --threads 8 -o ${bam.getSimpleName()}_targeted_depth.stats
    cat ${bam.getSimpleName()}_targeted_depth.stats | ${cmd1} > ${bam.getSimpleName()}_targeted_qc_cov.csv
    echo "min_cov,max_cov,mean_cov,targeted_above_mincov" > targeted_header.csv
    cat targeted_header.csv ${bam.getSimpleName()}_targeted_qc_cov.csv | ${awk3} > ${bam.getSimpleName()}_targeted_bam_qc.csv
    targeted_qc_info=\$(cat ${bam.getSimpleName()}_targeted_bam_qc.csv)
    targeted_bamfile="${bam},"
    targeted_full_info=\$targeted_bamfile\$targeted_qc_info
    echo \$targeted_full_info | jq -Rsn '
                         {"targeted_bam_qc":
                           [inputs
                            | . / "\n"
                            | (.[] | select(length > 0) | . / ",") as \$input
...
workflow wes_ETL_subKDK_grzSubmissionPreparation {
    ...
    main:
       ...
       bam_out = process_bamfile(id_bam_ch,grz_submission_dir_ch,wes_bedfile_ch,grz_bed_ch)
       ...
       sorted_normal_tumor_paired_bam_data = paired_bam_data.map{it ->
                                                                ...
                                                                def normalbam_lst = []
                                                                def tumorbam_lst = []
                                                                // this logic is due that 2 files are expected (2x N, 2x T)
...
workflow {
    ...
    wes_ETL_subKDK_grzSubmissionPreparation(target_dir_mvwes_ch,NextSeq_data_dir_ch,NovaSeq_data_dir_ch,nxf_outputdir_ch,grz_submission_dir_ch,hgnc_ch,wes_bedfile_ch,grz_bed_ch)
}
