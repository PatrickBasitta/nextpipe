...
process SAM_SORT_BAM {
    ...
    output:
        tuple val(sample_ID), path("sorted_${sample_ID}.bam"), emit: sorted_bam
...
process SAM_INDEX_AND_DEPTH {
    ...
    def awk2 = "awk -v OFS=',' '{sum+=\$4; ++n; if(\$4>=100){c++}} END{print sum/n, c/n}'"
    ...
    """
    samtools index ${sorted_bam} -o ${sorted_bam}.bai
    sambamba markdup -t 8 --tmpdir ${tmpdir} ${sorted_bam} ${sorted_bam.getSimpleName()}_markdup.bam
    mosdepth --by ${wes_bedfile} ${sorted_bam.getSimpleName()} ${sorted_bam.getSimpleName()}_markdup.bam
    samtools index ${sorted_bam.getSimpleName()}_markdup.bam -o ${sorted_bam.getSimpleName()}_markdup.bam.bai
    zcat ${sorted_bam.getSimpleName()}.regions.bed.gz | ${cmd1} > ${sorted_bam.getSimpleName()}_markdup_qc_cov.csv
