...
process FASTP_PAIRED {
    ...
    output:
        tuple val(sample_ID), path("${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed_${read1}"), path("${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed_${read2}"), emit: fp_trimmed_reads
        tuple val(sample_ID), path("${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed.json")      , emit: fastp_json
        tuple val(sample_ID), path("${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed.html")      , emit: fastp_html
    ...
    -i ${read1} \
    -I ${read2} \
    -o ${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed_${read1} \
    -O ${read1.getSimpleName()}_${read2.getSimpleName()}_fp_trimmed_${read2} \
    --thread ${task.cpus} \
    -j ${read1.getSimpleName()}_${read2.getSimpleName()}_${tissue}_fp_trimmed.json \
    -h ${read1.getSimpleName()}_${read2.getSimpleName()}_${tissue}_fp_trimmed.html
    """
}
...
process BWA_MEM {
    ...
    output:
        tuple val(sample_ID), path("${d_fp_trimmed_read1.getSimpleName()}_${d_fp_trimmed_read2.getSimpleName()}.bam"), emit: bam
    ...
    bwa mem -t ${task.cpus} ${idxbase} ${d_fp_trimmed_read1} ${d_fp_trimmed_read2} | samtools view -Shb -o ./${d_fp_trimmed_read1.getSimpleName()}_${d_fp_trimmed_read2.getSimpleName()}.bam
    """
}
...
process SAM_INDEX_AND_DEPTH {
    ...
    zcat ${sorted_bam.getSimpleName()}_markdup.per-base.bed.gz | ${awk1} > ${sorted_bam.getSimpleName()}_markdup_qc_cov.csv
    echo "mean_cov,targets_above_mincov" > header.csv
    cat header.csv ${sorted_bam.getSimpleName()}_markdup_qc_cov.csv | ${awk2} > ${sorted_bam.getSimpleName()}_markdup_bam_qc.csv
    ...
    zcat ${sorted_bam.getSimpleName()}_markdup_regions.regions.bed.gz | ${awk1} > ${sorted_bam.getSimpleName()}_targeted_markdup_qc_cov.csv
    echo "mean_cov,targets_above_mincov" > targeted_header.csv
    cat targeted_header.csv ${sorted_bam.getSimpleName()}_targeted_markdup_qc_cov.csv | ${awk2} > ${sorted_bam.getSimpleName()}_targeted_markdup_bam_qc.csv
